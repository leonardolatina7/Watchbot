...
function findWatchModel(title) {
  ...
  if (refMatch) {
    ...
    for (const [key, data] of Object.entries(WATCH_WEIGHTS)) {
      if (data.refs?.some(r => r.toUpperCase().includes(ref) || ref.includes(r.toUpperCase()))) {
        return { key, ...data, confidence: 'high' };
      }
    }
  }
  ...
  for (const [key, data] of Object.entries(WATCH_WEIGHTS)) {
    ...
    if (score > bestScore) { bestScore = score; bestMatch = { key, ...data, confidence: score >= 3 ? 'high' : 'medium' }; }
  }
  ...
  return { key:'generic', brand:'Generico', model:'Orologio oro/platino', metal: isPlatinum ? 'platinum' : '18k', pureMetalGrams: grams, confidence: 'low' };
}

module.exports = { WATCH_WEIGHTS, findWatchModel };
