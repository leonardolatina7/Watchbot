...
// Modello vision Groq: VERIFICA l'id corrente nella console Groq e mettilo in env.
// Default su un Llama vision del tier free al momento della scrittura.
...
// ritorna { gemme:[...], guarda:[...], verifiche:[...], visionUsate:n }
async function huntGems(listings, opts = {}) {
  ...
  const candidati = listings
    ...
    .map(l => ({ l, sc: textScore(l) }))
    .filter(x => !x.sc.kill && x.sc.score >= (opts.minScore ?? 3))
    .sort((a, b) => b.sc.score - a.sc.score)
    .slice(0, budget); // 3) tetto vision per restare nei limiti free
  ...
  return { gemme, guarda, verifiche, visionUsate };
}
