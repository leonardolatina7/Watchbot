...
// MOTORE VISION UNICO (Gemini primario → Groq fallback). Sostituisce la
// dipendenza dal solo Groq llama-4-scout (in deprecazione + 429 al minuto).
...
function classify(listing, vis, opts = {}) {
  ...
  const priceNow = Number(listing.price) || 0;
  if (!hit) {
    // La vista ha LETTO una marca ma non è nella tabella brandValue (~45 noti):
    // invece di buttarla — era LA causa di "poche gemme", il flip cieco mostrava
    // solo i marchi già noti — la mando come VERIFICA così la vedi e decidi tu.
    // Solo se la vista ha letto davvero una marca leggibile e il prezzo è in banda.
    const marca = String(vis?.marca || "").trim();
    if (marca && marca.length >= 2 && priceNow > 0 && priceNow <= lookMax) {
      return { tipo: "verifica", marca, prezzo: priceNow,
               nota: "marca non in tabella — grezzo cieco, valuta tu",
               redial: !!vis?.redial, confidenza: vis?.confidenza ?? null,
               calibro: vis?.calibro || null, materiale: vis?.materiale || null };
    }
    return null;
  }
...
async function huntGems(listings, opts = {}) {
  ...
  const minScore = opts.minScore ?? Number(process.env.FLIP_CIECO_MINSCORE || 1);
