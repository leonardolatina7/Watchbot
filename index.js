/**
 * Watch Price Bot v11.10 — "investitore a lungo termine" + DA STUDIARE
 *
 * Funziona SENZA API esterne a pagamento (eBay/Chrono24/Subito/Vinted RSS,
 * SerpAPI e Facebook opzionali). Novità v11.10 (sopra la v11.3):
...
// BLOCCO G — i marchi della watchlist Enciclopedia: pool ampio, a OGNI scan
// ne entrano 10 (5 fissi + 5 a rotazione), così nessuno resta mai indietro.
const QUERIES_G_CORE = ['Wittnauer chronograph vintage','Excelsior Park chronograph','Enicar Sherpa vintage','Gallet Multichron vintage','Movado vintage carica manuale'];
const QUERIES_G_POOL = [
  'Universal Geneve compax vintage','Nivada Grenchen Antarctic','Vulcain Cricket vintage','Aquastar Deepstar',
  'Enicar Super Divette','Eterna KonTiki vintage','Movado Kingmatic vintage','Czapek orologio',
  'Piaget oro vintage carica manuale','LeJour chronograph vintage','Helvetia chronograph vintage',
  'Wakmann chronograph','Lemania chronograph vintage','Gruen Curvex vintage','Minerva chronograph vintage',
  'Angelus Chronodato','orologio super compressor vintage','Vulcain Cricket oro',
  // ── indipendenti moderni (usato): rari ma ogni avvistamento conta ──
  'Urban Jurgensen','Czapek Quai des Bergues','Andersen Geneve','FP Journe usato',
  'De Bethune','Moser Endeavour','Universal Geneve Polerouter','Atelier Wen Perception',
  // ── fascia media (solo affari veri: usato a sconto profondo) ──
  'Tudor Black Bay 58','Tudor Pelagos usato','Grand Seiko usato','Cartier Santos usato',
  'JLC Reverso usato','Tudor Submariner vintage','Zenith El Primero usato',
  'Franck Muller Crazy Hours','Ulysse Nardin Freak','orologio ora saltante vintage','orologio seconde morte',
  // ── nuovi target di studio (vintage classico sottovalutato) ──
  'Heuer Autavia vintage','Heuer Camaro','Breitling Top Time vintage','Omega Seamaster vintage anni 60',
  'Omega Constellation vintage','IWC Ingenieur vintage','JLC Memovox vintage','Rado Captain Cook vintage',
  'Mido Multifort vintage','Bulova Accutron','Hamilton vintage automatico','Seiko 6139 chronograph',
  'Seiko 62MAS','LIP vintage','Baume Mercier vintage oro','Longines cronografo 30CH','Certina DS vintage',
  'Doxa Sub 300 vintage','Girard Perregaux gyromatic',
];
let scanCounter = 0;
function getGoldQueries() {
  ...
  const themed = rotating[scanCounter % rotating.length];
  // 5 query del pool G a rotazione (finestra scorrevole)
  const off = (scanCounter * 5) % QUERIES_G_POOL.length;
  const gRot = [];
  for (let i = 0; i < 5; i++) gRot.push(QUERIES_G_POOL[(off + i) % QUERIES_G_POOL.length]);
  return Array.from(new Set([...QUERIES_G_CORE, ...gRot, ...themed]));
}
...
async function runGoldScan(mode = 'all') {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  console.log(`\n[SCAN v11.10 ${mode==='subito'?'SUBITO':'COMPLETA'}] Oro: €${gold.toFixed(2)}/g | Platino: €${platinum.toFixed(2)}/g`);
  ...
  for (const query of activeQueries) {
    try {
      const sources = mode === 'subito'
        ...
        : [ searchSubito, searchEbayAllCountries, searchSerpAPILocal,
            (qq)=>searchSerpAPI(qq,'it'), searchMarktplaats, searchVinted ];
      let all = [];
...
app.post('/api/telegram/test',(req,res) => { tg(`⌚ <b>Watch Price Bot v11.10</b> — Test OK! 🟢`,req.body.chatId); res.json({ok:true}); });

app.get('/api/status',async(req,res) => {
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  res.json({
    status:'online',version:'11.10',
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  console.log(`\n⌚ Watch Price Bot v11.10 — porta ${PORT}`);
  ...
  if(process.env.TELEGRAM_TOKEN&&process.env.TELEGRAM_CHAT_ID){
    ...
    await tg(
      ...
      `\n<b>NOVITÀ v11.10:</b> raddoppiati i marchi DA STUDIARE (ora 49: entrano Omega, IWC, JLC, Heuer, Seiko, GP, Longines, Rado, Bulova, Hamilton e altri) e le query dedicate. Dovresti vedere molti più 📚 DA STUDIARE.\n`+
