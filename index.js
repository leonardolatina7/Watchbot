/**
 * Watch Price Bot v12.2 — RICERCA EBAY GRATIS + ANALISI GROQ (zero costi API)
 *
...
const HARD_NOT_A_WATCH_PATTERNS = [
  ...
  /\b(catalogo|catalogue|katalog|rivista|magazine|brochure|d\u00e9pliant|depliant|libro\s+su|book\s+about)\b/i,
];
...
async function runGoldScan(mode = 'all') {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  console.log(`\n[SCAN v12.2 ${mode==='subito'?'SUBITO':'COMPLETA'}] Oro: €${gold.toFixed(2)}/g | Platino: €${platinum.toFixed(2)}/g`);

  let foundArb=0, foundNear=0, foundVintage=0, foundStudy=0, foundInvestor=0;
  ...
  console.log(`[SCAN] Fine: ${foundArb} arbitraggi, ${foundNear} trattabili, ${foundVintage} vintage, ${foundInvestor} investitore, ${foundStudy} da studiare`);
  await tg(
...
app.get('/api/diagnostica', async (req, res) => {
  ...
  out._query = q;
  // ── AUTODIAGNOSI v12.2: dice subito se le ultime novità sono ONLINE ──
  out._versione = '12.2';
  out._novita = {
...
app.post('/api/telegram/test',(req,res) => { tg(`⌚ <b>Watch Price Bot v12.2</b> — Test OK! 🟢`,req.body.chatId); res.json({ok:true}); });

app.get('/api/status',async(req,res) => {
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  res.json({
    status:'online',version:'12.2',
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  console.log(`\n⌚ Watch Price Bot v12.2 — porta ${PORT}`);
