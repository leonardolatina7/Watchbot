/**
 * Watch Price Bot v11.7 — "investitore a lungo termine" + DA STUDIARE
 *
 * Funziona SENZA API esterne a pagamento (eBay/Chrono24/Subito/Vinted RSS,
 * SerpAPI e Facebook opzionali). Novità v11.7 (sopra la v11.3):
...
async function runGoldScan(mode = 'all') {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  console.log(`\n[SCAN v11.7 ${mode==='subito'?'SUBITO':'COMPLETA'}] Oro: €${gold.toFixed(2)}/g | Platino: €${platinum.toFixed(2)}/g`);
...
async function checkObservedMatch(item, priceEur) {
  ...
  for (const obs of db.observed) {
    ...
    if (!obs.threshold) continue; // senza soglia non scatta nulla (la metti tu)
    // match per URL identico, oppure per marchio presente nel titolo
    const sameUrl = obs.url && item.url && obs.url === item.url;
    const brandHit = obs.brand && titleN.includes(normTxt(obs.brand));
    if (!sameUrl && !brandHit) continue;
    if (priceEur > obs.threshold) continue;
    // anti-doppione: non riavviso lo stesso url sotto soglia entro 24h
    ...
    saveState();
    await tg(
      `\u{1F441}\uFE0F\u{1F4C9} <b>OSSERVATO SCESO SOTTO SOGLIA</b>\n\n`+
      `Stavi tenendo d'occhio: <b>${obs.brand||obs.label||'pezzo'}</b>\n`+
      `\u{1F3AF} Tua soglia: <b>\u20AC${Number(obs.threshold).toLocaleString('it-IT')}</b>\n`+
      `\u231A ${item.title?.slice(0,65)}\n`+
      `\u{1F4B0} Ora chiede: <b>\u20AC${priceEur.toLocaleString('it-IT')}</b>\n`+
      `\u{1F3EA} ${item.platform}${item.location?` \u00B7 \u{1F4CD} ${item.location}`:''}\n\n`+
      `<a href="${item.url}">\u{1F449} VEDI ANNUNCIO</a>\n`
    );
  }
...
//   /api/osserva?b=Girard%20Perregaux&t=GP%20cal%20640%20oro&p=1400&soglia=750
//   (se ometti soglia, lo registro SENZA soglia: la imposti dopo con /api/osserva/soglia)
app.get('/api/osserva', (req, res) => {
  ...
  const sogliaTxt = threshold
    ...
    : `Nessuna soglia ancora impostata. Aprila e mettila tu:<br><br><textarea readonly style="width:100%;height:54px;background:#1b2230;color:#f3f1ea;border:1px solid #3a4456;border-radius:8px;padding:8px;font-size:13px">${SELF_URL}/api/osserva/soglia?id=${rec.id}&soglia=METTI_PREZZO</textarea>`;
...
app.post('/api/telegram/test',(req,res) => { tg(`⌚ <b>Watch Price Bot v11.7</b> — Test OK! 🟢`,req.body.chatId); res.json({ok:true}); });

app.get('/api/status',async(req,res) => {
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  res.json({
    status:'online',version:'11.7',
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  if(process.env.TELEGRAM_TOKEN&&process.env.TELEGRAM_CHAT_ID){
    ...
    await tg(
      ...
      `\n<b>NOVITÀ v11.7:</b> entra la FASCIA MEDIA (Tudor, Grand Seiko, JLC, Cartier): passa il filtro vintage ma SOLO come affare vero a sconto concreto — mai come studio. Query dedicate (BB58, Pelagos, GS usato, Santos, Reverso, El Primero).\n`+
      `Tieni d'occhio un pezzo scartato: tocca 👁️ Osserva sotto ogni alert e gli metti tu la soglia.\n\nPrima scansione tra 90 secondi...`
    );
  }
