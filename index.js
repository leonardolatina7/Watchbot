...
const longines = require('./longinesCalibers'); // ── MODULO LONGINES (calibri + datazione seriale + anti-franken) ──
...
function gistSaveDebounced(payload) {
  ...
  _gistTimer = setTimeout(async () => {
    try {
      ...
      console.log(`[GIST] salvato (${(payload.dismissedUrls||[]).length} scartati, ${(payload.blacklistBrands||[]).length} marchi)`);
...
async function loadStateGist() {
  ...
  console.log(`[GIST] caricato: ${db.blacklistBrands.length} marchi bloccati, ${db.dismissedUrls.length} annunci scartati, ${Object.keys(db.alertedFps).length} impronte`);
}
...
const SCAN_FLOOR = parseInt(process.env.SCAN_FLOOR || '70');
...
const IS_WATCH_OVERRIDE = /\b(orologio|orologi|watch|wristwatch|horloge|montre|armbanduhr|uhr\b|reloj|chronograph|chronographe|cronografo|automatic|automatico|automatique|carica\s*manuale|hand[\s-]*wind|datejust|seamaster|speedmaster|submariner|calibr[eo]|caliber|jewels?|rubis|movement|movimento|quadrante|\bdial\b|ghiera|bracelet\s*watch)\b/i;
...
function detectMetal(t) {
  ...
  const partialGold =
    ...
    || /\b(acciaio\s*[\/e]+\s*oro|oro\s*[\/e]+\s*acciaio|steel\s*(and|&|\/)\s*gold|gold\s*(and|&|\/)\s*steel|two.?tone|bicolor|bicolore|acier\s*or|stahl\s*gold)\b/i.test(s)
    ...
    // acciaio E un pezzo-in-oro nello stesso titolo = quasi sempre bicolore/trim oro, non cassa massiccia
    || (steelWord.test(s) && goldPart.test(s) && goldWord.test(s))
...
async function searchLeboncoin(query) {
  try {
    ...
    $('[data-qa-id="aditem_container"],[class*="AdCard"],[class*="adCard"]').each((i,el) => {
      ...
      const link = $el.find('a').first().attr('href');
      if (title&&price>=SCAN_FLOOR) results.push({ platform:'Leboncoin 🇫🇷', title, price, currency:'EUR',
...
async function searchOlxPl(query) {
  try {
    ...
    const r = await axios.get('https://www.olx.pl/api/v1/offers/', {
      params: { query, category_id: 99, 'filter_float_price:from': 850, sort_by: 'filter_float_price:asc', limit: 20 },
...
async function searchOlxRo(query) {
  try {
    ...
    const r = await axios.get('https://www.olx.ro/api/v1/offers/', {
      params: { query, category_id: 99, 'filter_float_price:from': 900, sort_by: 'filter_float_price:asc', limit: 20 },
...
//    scraping HTML leggero (fragile per natura, come gli altri scraper HTML).
//    Estraggono titolo + prezzo dai listing. Prezzi PLN / RON.
async function searchSprzedajemyPl(query) {
  try {
    ...
    const results = [];
    // blocchi offerta: titolo in <h2 ...>...</h2>, prezzo "1 234 zł"
    const re = /<a[^>]+class="[^"]*offimg[^"]*"[^>]+title="([^"]+)"[^>]*href="([^"]+)"[\s\S]{0,800}?(\d[\d \u00a0]{2,})\s*zł/gi;
    let m;
    while ((m = re.exec(html)) && results.length < 14) {
      const title = m[1].replace(/&[^;]+;/g,' ').trim();
      const pln = parseFloat(m[3].replace(/[ \u00a0]/g,'')) || 0;
      const url = m[2].startsWith('http') ? m[2] : 'https://sprzedajemy.pl'+m[2];
      if (title && pln >= 800) results.push({ platform:'Sprzedajemy 🇵🇱', title, price:pln, currency:'PLN', url, location:'' });
    }
...
async function searchOkaziiRo(query) {
  try {
    ...
    const html = String(r.data||'');
    const results = [];
    const re = /<a[^>]+href="([^"]+)"[^>]*class="[^"]*product[^"]*"[^>]*>[\s\S]{0,300}?title="([^"]+)"[\s\S]{0,600}?(\d[\d.,]+)\s*(?:lei|RON)/gi;
    let m;
    while ((m = re.exec(html)) && results.length < 14) {
      const title = m[2].replace(/&[^;]+;/g,' ').trim();
      const ron = parseFloat(m[3].replace(/\./g,'').replace(',','.')) || 0;
      const url = m[1].startsWith('http') ? m[1] : 'https://www.okazii.ro'+m[1];
      if (title && ron >= 900) results.push({ platform:'Okazii 🇷🇴', title, price:ron, currency:'RON', url, location:'' });
    }
  ...
  for (const rawQuery of activeQueries) {
    try {
      ...
      for (const item of all) {
        ...
        const priceEur = Math.round(await toEur(item.price, item.currency));
        if (priceEur<SCAN_FLOOR) continue;
        ...
        if (db.dismissedUrls.includes(nu) || db.dismissedFps.includes(fp)) continue;
        ...
        if (looksLikeNotAWatch(item.title)) { console.log(`[NON-OROLOGIO] salto (occhiali/penna/gioiello): ${item.title?.slice(0,45)}`); continue; }
...
// ══════════════ API ROUTES ══════════════
app.get('/api/versione', (req, res) => res.json({ versione: '12.10', mercati: ['eBay','Subito','Vinted','Chrono24','Catawiki','Leboncoin','Wallapop','Ricardo','Marktplaats','OLX.pl','OLX.ro','Allegro.pl','Sprzedajemy.pl','Okazii.ro','MercadoLibre MX','MercadoLibre BR'] }));
