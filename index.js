/**
 * Watch Price Bot v11.13 — "investitore a lungo termine" + DA STUDIARE
 *
...
async function searchSubito(query) {
  try {
    ...
    if (m) {
      try {
        ...
        for (const ad of ads.slice(0, 15)) {
          ...
          let price = 0;
          const feats = ad.features || ad.params || {};
          const pStr = JSON.stringify(feats).match(/"?prezzo"?\s*[:,]\s*"?(\d[\d.,]*)/i);
          if (pStr) price = parsePrice(pStr[1]);
          if (!price && ad.price) price = parsePrice(String(ad.price));
          const link = ad.urls?.default || ad.url || '';
          if (title && price>=200 && link) results.push({ platform:'Subito.it 🇮🇹', title, price, currency:'EUR', isLocal:true, location: ad.geo?.town?.value||'', url:link });
        }
      } catch (e) { console.error('[Subito JSON]', e.message); }
    }
    // fallback HTML se il JSON non ha prodotto nulla
    if (!results.length) {
      parseAndFree(r.data, ($) => {
    ...
    if (results.length) console.log(`[Subito] ${query}: ${results.length}`);
...
// ── MODALITA TEST A SECCO (dry-run) ──
// Quando attiva, il bot fa TUTTO lo scan e TUTTI i filtri ma NON chiama mai
// Claude: raccoglie cosa AVREBBE analizzato e lo manda come report. Zero token.
// Serve a verificare il pre-filtro senza spendere. Si accende con la env
// DRY_RUN=true oppure a caldo via /api/dryrun/on (e /off).
let DRY_RUN = String(process.env.DRY_RUN || '').toLowerCase() === 'true';
let dryRunLog = []; // {title, price, platform, url} di cosa sarebbe andato a Claude
...
async function runGoldScan(mode = 'all') {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  console.log(`\n[SCAN v11.16 ${mode==='subito'?'SUBITO':'COMPLETA'}] Oro: €${gold.toFixed(2)}/g | Platino: €${platinum.toFixed(2)}/g`);
  ...
  for (const rawQuery of activeQueries) {
    try {
      ...
      const sources = geoMarket
        ? ( // ── CACCIA GEO: cerco il modello nel mercato dove e FREDDO ──
            geoMarket === 'it'
              ? [ searchSubito, searchSerpAPILocal, searchEbaySerpAPI ]   // tedeschi pescati in IT
              : geoMarket === 'de'
                ? [ (qq)=>searchSerpAPI(qq,'de'), searchEbaySerpAPI, searchMarktplaats ] // italiani/FR pescati in DE
                : [ searchEbaySerpAPI, searchChrono24RSS, searchMarktplaats ] // JP/US pescati in EU
          )
        : mode === 'subito'
        ? [ searchSubito, searchSerpAPILocal ]
        : italiaFirst
          ? [ // ── Italia-first: solo sorgenti italiane ──
              searchSubito,          // 🇮🇹 annunci privati IT
              searchSerpAPILocal,    // 🇮🇹 Google Shopping IT
              searchEbaySerpAPI,     // eBay (SerpAPI) — il dominio IT pesa qui
              searchChrono24RSS,     // Chrono24 IT
            ]
          : [ // ── Full estero + IT: copertura ampia ──
              searchMarktplaats,
              searchSubito,
              searchChrono24RSS,
              searchEbaySerpAPI,
              searchSerpAPILocal,
              (qq)=>searchSerpAPI(qq,'de'),
              (qq)=>searchSerpAPI(qq,'fr'),
              searchVinted,
            ];
      ...
      for (const item of all) {
        ...
        if (claudeAnalyst.isConfigured()) {
          // MODALITA TEST A SECCO: applico solo il pre-filtro (zero token) e
          // registro cosa sarebbe andato a Claude, senza chiamarlo davvero.
          if (DRY_RUN) {
            if (claudeAnalyst.passesPreFilter(item.title)) {
              dryRunLog.push({ title: item.title, price: priceEur, platform: item.platform, url: item.url });
            }
            seenUrls.add(nu);
            continue;
          }
  ...
  // ── REPORT MODALITA TEST A SECCO ──
  if (DRY_RUN) {
    const n = dryRunLog.length;
    const sample = dryRunLog.slice(0, 25).map((d,i)=>`${i+1}. <b>\u20AC${(d.price||0).toLocaleString('it-IT')}</b> \u00B7 ${(d.title||'').slice(0,70)} <i>(${d.platform})</i>`).join('\n');
    await tg(
      `\u{1F9EA} <b>TEST A SECCO \u2014 nessun token speso</b>\n\n`+
      `Ho fatto tutto lo scan e i filtri. <b>${n}</b> annunci avrebbero superato il pre-filtro e sarebbero andati a Claude per l'analisi.\n\n`+
      (n ? `\u{1F50D} <b>Ecco i primi ${Math.min(n,25)}</b> (controlla se c'\u00E8 ancora spazzatura):\n${sample}\n\n` : `\u2705 Nessun annuncio \u00E8 passato: o niente di rilevante in giro, o il filtro \u00E8 molto severo.\n\n`)+
      `\u{1F4A1} Se vedi ancora robaccia (occhiali, acciaio-oro moderno, ecc.) dimmelo e stringo il filtro. Quando sei soddisfatto: <code>/api/dryrun/off</code> per riattivare Claude.`
    );
    dryRunLog = []; // reset per il prossimo scan
    return { foundArb:0, foundNear:0, foundVintage:0, foundStudy:0, dryRun:n };
  }
...
// ── MODALITA TEST A SECCO ──
app.get('/api/dryrun/on',(req,res)=>{ DRY_RUN=true; dryRunLog=[]; res.send(fbPage('\u{1F9EA} Test a secco ATTIVO','Lo scan ora NON chiama Claude: ti manda solo il report di cosa avrebbe analizzato. Zero token. Lancia /api/gold-scan per provare.')); });
app.get('/api/dryrun/off',(req,res)=>{ DRY_RUN=false; res.send(fbPage('\u2705 Test a secco SPENTO','Claude \u00E8 di nuovo attivo: lo scan torna a fare l\'analisi vera.')); });
app.get('/api/dryrun/status',(req,res)=>res.json({ dryRun:DRY_RUN, ultimoLog:dryRunLog.length }));
...
app.post('/api/telegram/test',(req,res) => { tg(`⌚ <b>Watch Price Bot v11.16</b> — Test OK! 🟢`,req.body.chatId); res.json({ok:true}); });

app.get('/api/status',async(req,res) => {
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  res.json({
    status:'online',version:'11.16',
    ...
    claudeConfigured:claudeAnalyst.isConfigured(), claudeUsage:claudeAnalyst.getUsage(), dryRun:DRY_RUN,
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  if(process.env.TELEGRAM_TOKEN&&process.env.TELEGRAM_CHAT_ID){
    ...
    await tg(
      ...
      (claudeOff
        ? `\n🔴 <b>ATTENZIONE: Claude è SPENTO.</b>\nManca ANTHROPIC_API_KEY su Render. Senza, il bot scarta quasi tutto e trovi 0 occasioni. Aggiungila in Environment e si riavvia da solo.\n`
        : `🤖 Analisi Claude: <b>ATTIVA</b>\n`)+
      `\n<b>NOVITÀ v11.16 \u2014 STOP agli sprechi di token:</b>\n`+
      `\u{1F6AB} Pre-filtro rinforzato: occhiali, gioielli, accessori, marchi top moderni (Cartier/Rolex/Patek recenti) e acciaio-oro senza interesse vintage vengono buttati PRIMA di Claude, a costo zero. Niente piu token bruciati su robaccia.\n`+
      `\u{1F9EA} Modalita TEST A SECCO: /api/dryrun/on fa girare scan e filtri SENZA chiamare Claude e ti manda il report di cosa avrebbe analizzato. Zero token. /api/dryrun/off per riattivare.\n`+
