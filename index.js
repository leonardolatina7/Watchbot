/**
 * Watch Price Bot v11.3 — "investitore a lungo termine" + DA STUDIARE
 *
 * Funziona SENZA API esterne a pagamento (eBay/Chrono24/Subito/Vinted RSS,
 * SerpAPI e Facebook opzionali). Novità v11.3 (sopra la v11.2):
...
let db = {
  ...
  dismissedUrls: [],
  liked: [],
  observed: [],   // ── NOVITÀ: pezzi "osservati ma non comprati" (soglia manuale)
};
let _id = Date.now();
const nid = () => ++_id;
...
function loadState() {
  ...
  try {
    if (fs.existsSync(STATE_FILE)) {
      ...
      console.log(`[STATE] Caricati: ${db.blacklistBrands.length} marchi bloccati, ${db.dismissedUrls.length} annunci scartati, ${db.liked.length} piaciuti, ${db.observed.length} osservati`);
    }
...
// BLOCCO G — i marchi-azienda della watchlist investimento + Enciclopedia (sempre attivo)
const QUERIES_BLOCK_G = ['Czapek orologio','Universal Geneve compax vintage','Nivada Grenchen Antarctic','Vulcain Cricket oro','Aquastar Deepstar','Enicar Super Divette','Excelsior Park Monte Carlo','Wittnauer Valjoux 72','Movado Kingmatic vintage','Eterna KonTiki vintage'];
let scanCounter = 0;
function getGoldQueries() {
  scanCounter++;
  // I 6 blocchi tematici ruotano; il blocco G (marchi Enciclopedia) è SEMPRE
  // incluso, così Wittnauer/Excelsior Park/Enicar minori & co. li cerco a ogni scan.
  ...
  // Unisco G + blocco a rotazione, deduplicando, e taglio per non gonfiare troppo.
  return Array.from(new Set([...QUERIES_BLOCK_G, ...themed]));
}
...
async function runGoldScan(mode = 'all') {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  console.log(`\n[SCAN v11.3 ${mode==='subito'?'SUBITO':'COMPLETA'}] Oro: €${gold.toFixed(2)}/g | Platino: €${platinum.toFixed(2)}/g`);
...
app.post('/api/telegram/test',(req,res) => { tg(`⌚ <b>Watch Price Bot v11.3</b> — Test OK! 🟢`,req.body.chatId); res.json({ok:true}); });

app.get('/api/status',async(req,res) => {
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  res.json({
    status:'online',version:'11.3',
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  if(process.env.TELEGRAM_TOKEN&&process.env.TELEGRAM_CHAT_ID){
    ...
    await tg(
      ...
      `\n<b>NOVITÀ v11.3:</b> ora i marchi della tua Enciclopedia (Wittnauer, Excelsior Park, Enicar minori, Gallet, Movado pre-quarzo, Vulcain, EPSA…) li cerco a ogni scan e te li mando come 📚 DA STUDIARE anche quando non sono affari, se il prezzo è in linea col mercato.\n`+
