...
const QUERIES_BLOCK_A = [
  'Nivada Grenchen chronograph vintage',
  ...
  'Doxa Sub vintage',
  'orologio cronografo vintage svizzero',
];
const QUERIES_BLOCK_B = [
  'Excelsior Park chronograph',
  'Vulcain Cricket vintage',
  'Favre-Leuba Bivouac vintage',
  'Longines chronograph 30CH vintage',
  'Tissot Seastar chronograph Lemania',
  'LIP chronograph vintage',
  'Movado chronograph vintage',
  'Lemania chronograph vintage',
  'Eberhard Extra Fort vintage',
  'orologio oro 18k vintage svizzero',
];
const QUERIES_BLOCK_C = [
  ...
  'Sandoz chronograph Valjoux vintage',
  'Waltham diver chronograph vintage',
  'Wittnauer chronograph vintage',
  'cronografo Valjoux 72 vintage',
  'orologio subacqueo vintage anni 60',
];
let scanCounter = 0;
function getGoldQueries() {
  scanCounter++;
  const blocks = [QUERIES_BLOCK_A, QUERIES_BLOCK_B, QUERIES_BLOCK_C];
  return blocks[scanCounter % 3];
}
  ...
  for (const query of activeQueries) {
    try {
      ...
      for (const item of all) {
        if (!item.url||seenUrls.has(item.url)) continue;
        const priceEur = Math.round(await toEur(item.price, item.currency));
        if (priceEur<200) continue; // soglia bassa per strategia volumi
        ...
        // Strategia volumi: analizza anche pezzi economici (da 200€).
        // Claude identifica, valuta condizioni dalla foto, e segnala
        // anche piccoli margini (200→300 va bene), purché sia un orologio vero.
        if (claudeAnalyst.isConfigured() && priceEur >= 200 && !vintage) {
          ...
          if (ai && ai.isInteresting) {
            ...
            const sleeperTag = (ai.isSleeper && !ai.isGrail) ? '💎 SOTTOVALUTATO ' : '';
            ...
            await tg(
              `🤖 <b>${grailTag}${sleeperTag}AFFARE ${ai.dealStrength||''} ${strengthEmoji}</b>\n\n`+
              `⌚ ${item.title?.slice(0,65)}\n`+
