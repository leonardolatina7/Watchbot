      ...
      for (const item of all) {
        ...
        if (claudeAnalyst.isConfigured()) {
          ...
          if (ai && ai.isInteresting) {
            ...
            await tg(
              `\u{1F916} ${grailTag}${sleeperTag}<b>AFFARE ${ai.dealStrength||''} ${strengthEmoji}</b>\n\n`+
        ...
        if (metal.isArbitrage) {
          ...
          // Alert SOLO se peso certificato (referenza nota). I pesi stimati
          // sono inaffidabili: salvati per il sito ma NON inviati, per non
          // intasare Telegram con arbitraggi dubbi.
          if (metal.confidence === 'high') {
            const emoji = metal.metal==='platinum'?'🔘':'🥇';
            const name = metal.metal==='platinum'?'PLATINO':'ORO 18K';
            await tg(
              `${emoji} <b>ARBITRAGGIO ${name}</b> (peso certificato)\n\n`+
              `⌚ ${item.title?.slice(0,65)}\n`+
              ...
              `\n🔍 <a href="${metal.verifyLink||'https://watchcharts.com'}">Verifica peso</a>\n`+
              `\n<a href="${item.url}">👉 VEDI ANNUNCIO</a>`
            );
          }
          console.log(`[ARB] ${item.title?.slice(0,40)} €${priceEur} (conf:${metal.confidence}, alert:${metal.confidence==='high'?'si':'no'})`);
        } else if (metal.isNear) {
          // Trattabili: salvati per il sito, MAI su Telegram.
          db.nearArbitrage.push(entry); foundNear++;
          console.log(`[NEAR] ${item.title?.slice(0,40)} €${priceEur} (solo sito)`);
        }
      }
