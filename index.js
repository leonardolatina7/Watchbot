        ...
        if (claudeAnalyst.isConfigured() && priceEur >= 800 && !vintage) {
          ...
          if (ai && ai.isInteresting) {
            ...
            await tg(
              ...
              `🏪 ${item.platform}${item.location?` · 📍 ${item.location}`:''}\n\n`+
              `💡 ${ai.reasoning||''}\n\n`+
              `⚠️ Stima da titolo, non da foto. Verifica condizioni e originalità.\n\n`+
              `<a href="${item.url}">👉 VEDI ANNUNCIO</a>`
            );
            console.log(`[CLAUDE 🤖] ${ai.brand} ${ai.model} €${priceEur} (stima €${ai.valueLow}, ${ai.confidence})`);
            continue;
          }
        }
