...
// ── FX ──
let fx = { USD:0.92, GBP:1.17, CHF:1.05, PLN:0.23 }, fxF = 0;
async function toEur(price, currency) {
  if (!price||currency==='EUR') return price;
  if (Date.now()-fxF>3600000) {
    try { const r=await axios.get('https://api.frankfurter.app/latest?from=EUR&to=USD,GBP,CHF,PLN',{timeout:5000}); fx={USD:1/r.data.rates.USD,GBP:1/r.data.rates.GBP,CHF:1/r.data.rates.CHF,PLN:1/r.data.rates.PLN}; fxF=Date.now(); } catch {}
  }
...
async function searchAll(query) {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  const [ebay, chrono, catawiki, subito, leboncoin, vestiaire, watchfinder, vinted, wallapop, ricardo, marktplaats, olxpl, serp, serpLocal, fb] = await Promise.allSettled([
    ...
    searchWallapop(query), searchRicardo(query), searchMarktplaats(query), searchOlxPl(query),
