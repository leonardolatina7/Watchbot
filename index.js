/**
 * Watch Price Bot v11.11 — "investitore a lungo termine" + DA STUDIARE
 *
 * Funziona SENZA API esterne a pagamento (eBay/Chrono24/Subito/Vinted RSS,
 * SerpAPI e Facebook opzionali). Novità v11.11 (sopra la v11.3):
...
async function runGoldScan(mode = 'all') {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  console.log(`\n[SCAN v11.11 ${mode==='subito'?'SUBITO':'COMPLETA'}] Oro: €${gold.toFixed(2)}/g | Platino: €${platinum.toFixed(2)}/g`);
...
app.post('/api/telegram/test',(req,res) => { tg(`⌚ <b>Watch Price Bot v11.11</b> — Test OK! 🟢`,req.body.chatId); res.json({ok:true}); });

app.get('/api/status',async(req,res) => {
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  res.json({
    status:'online',version:'11.11',
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  console.log(`\n⌚ Watch Price Bot v11.11 — porta ${PORT}`);
  ...
  if(process.env.TELEGRAM_TOKEN&&process.env.TELEGRAM_CHAT_ID){
    ...
    await tg(
      ...
      `\n<b>NOVITÀ v11.11:</b> ricerca MIRATA all'Enciclopedia — niente più query generiche che sprecano analisi, ora ogni ricerca punta a marchio+modello+calibro preciso (62 query enciclopediche dalla watchlist) e ai "parenti poveri" delle leggende. Pre-filtro anti-fumo: sveglie, cinturini, smartwatch e repliche scartati a costo zero, senza bruciare analisi. Arrosto, non fumo.\n`+
