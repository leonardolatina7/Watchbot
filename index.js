/**
 * Watch Price Bot v12.1 — RICERCA EBAY GRATIS + ANALISI GROQ (zero costi API)
 *
...
async function runGoldScan(mode = 'all') {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  console.log(`\n[SCAN v12.1 ${mode==='subito'?'SUBITO':'COMPLETA'}] Oro: €${gold.toFixed(2)}/g | Platino: €${platinum.toFixed(2)}/g`);
...
app.get('/api/diagnostica', async (req, res) => {
  ...
  out._query = q;
  // ── AUTODIAGNOSI v12.1: dice subito se le ultime novità sono ONLINE ──
  out._versione = '12.1';
  out._novita = {
...
app.post('/api/telegram/test',(req,res) => { tg(`⌚ <b>Watch Price Bot v12.1</b> — Test OK! 🟢`,req.body.chatId); res.json({ok:true}); });

app.get('/api/status',async(req,res) => {
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  res.json({
    status:'online',version:'12.1',
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  console.log(`\n⌚ Watch Price Bot v12.1 — porta ${PORT}`);
