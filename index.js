...
function loadState() {
  ...
  try {
    if (fs.existsSync(STATE_FILE)) {
      ...
      console.log(`[STATE] Caricati: ${db.blacklistBrands.length} marchi bloccati, ${db.dismissedUrls.length} annunci scartati`);
    }
...
function feedbackLinks(brand, url) {
        ...
        if (claudeAnalyst.isConfigured()) {
          ...
          if (ai && ai.isInteresting) {
            ...
            await tg(
              ...
              `\u2796\u2796\u2796\n`+
              feedbackLinks(ai.brand, item.url)
            );
...
cron.schedule('30 * * * *',()=>runGoldScan('subito').catch(()=>{}));   // ogni ora, al minuto 30
cron.schedule('0 1,6,11,16,21 * * *',()=>runGoldScan('all').catch(()=>{})); // completa ogni 5h
