...
async function searchEbayAPI(query) {
  ...
  for (const market of ['EBAY_IT','EBAY_FR','EBAY_DE','EBAY_GB','EBAY_ES']) {
    ...
    const floor = cur === 'GBP' ? 170 : 200;
    try {
      const r = await axios.get('https://api.ebay.com/buy/browse/v1/item_summary/search',{
        params:{q:query,category_ids:'31387',sort:'price',limit:50,filter:`price:[${floor}..],priceCurrency:${cur}`},
...
async function fetchMarktplaatsCieco(query, maxPrice) {
  try {
    ...
    for (const i of listings) {
      ...
      // campo immagine difensivo: provo i nomi più probabili; se nessuno c'è la
      // vision salta l'item senza rompere nulla (degradazione graziosa).
      const imageUrl = i.pictures?.[0]?.extraExtraLargeUrl || i.pictures?.[0]?.largeUrl || i.pictures?.[0]?.url || i.imageUrls?.[0] || '';
      if (!title || !url || price < CIECO_FLOOR || price > maxPrice) continue;
...
cron.schedule('0 */2 * * *',()=>runGoldScan('all').catch(e=>console.error('[SCAN]',e.message)));
...
cron.schedule('30 */3 * * *',()=>runFlipCiecoScan().catch(e=>console.error('[FLIP CIECO]',e.message)));
