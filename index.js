...
// ══════════════ API ROUTES ══════════════
app.get('/api/versione', (req, res) => res.json({ versione: '12.12', mercati: ['eBay','Subito','Vinted','Chrono24','Catawiki','Leboncoin','Wallapop','Ricardo','Marktplaats','OLX.pl','OLX.ro','Allegro.pl','Sprzedajemy.pl','Okazii.ro','MercadoLibre MX','MercadoLibre BR'] }));
app.get('/api/diagnostica', (req, res) => {
  if (!lastScraperDiag) return res.json({ stato: 'nessuno scan ancora eseguito da questo avvio', suggerimento: 'aspetta il prossimo giro orario o chiama /api/scan-ora se disponibile' });
  const c = lastScraperDiag.counts;
  const morti = Object.keys(c).filter(k => typeof c[k] === 'string' || c[k] === 0);
  const vivi = Object.keys(c).filter(k => typeof c[k] === 'number' && c[k] > 0);
  res.json({
    ultimoScan: lastScraperDiag.at,
    query: lastScraperDiag.query,
    conteggioPerPiazza: c,
    piazzeVive: vivi,
    piazzeAZeroOrotte: morti,
    nota: 'string tipo "rej(...)" = scraper in errore (piazza bloccata/morta). 0 = nessun annuncio reale per QUELLA query. Numero = annunci scaricati.'
  });
});
