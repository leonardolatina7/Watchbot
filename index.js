...
async function runGoldScan() {
  ...
  for (const query of activeQueries) {
    try {
      ...
      const [ebay, chrono, catawiki, subito, leboncoin, vinted, wallapop, ricardo, serp] = await Promise.allSettled([
        searchEbayAllCountries(query),
        searchChrono24RSS(query),
        searchCatawiki(query),
        searchSubito(query),
...
// ── CRON ─────────────────────────────────────────────────────
cron.schedule('0 */8 * * *',()=>runGoldScan().catch(()=>{}));
cron.schedule('0 */8 * * *',()=>runHypeScan().catch(()=>{}));
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  setTimeout(()=>runHypeScan().catch(()=>{}),5*60*1000);
