...
async function searchMarktplaats(query) {
  // categoryId=625 = Sieraden, Tassen en Uiterlijk > Horloges su Marktplaats.
  // Il vecchio 363 era troppo largo (arte/antiquariato incluso).
  // Filtro titolo in caso qualcosa di non-orologio passi lo stesso.
  const NON_OROLOGIO = [
    'schilderij','schilderijen','aquarium','aquaria','gemälde','peinture',
    'lithografie','tekening','ets ','gravure','poster ','fotografie',
    'kunst |','| kunst','sieraden set','handtas','parfum','bril ',
  ];
  try {
    await sleep(1000+Math.random()*500);
    const r = await axios.get(`https://www.marktplaats.nl/lrp/api/search?query=${encodeURIComponent(query)}&categoryId=625&priceFrom=200&sortBy=PRICE_ASC&limit=12`, { headers:{'User-Agent':rUA(),'Accept':'application/json'}, timeout:12000 });
    ...
    return listings.slice(0,12).map(i=>({
      ...
      if (i.price < 200 || !i.title) return false;
      const tl = i.title.toLowerCase();
      if (NON_OROLOGIO.some(k => tl.includes(k))) {
        console.log(`[Marktplaats] scartato (non-orologio): ${i.title.slice(0,60)}`);
        return false;
      }
      return true;
    });
