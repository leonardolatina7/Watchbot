...
function looksLikeNotAWatch(title) {
  const t = String(title || '');
  if (!NOT_A_WATCH_PATTERNS.some(re => re.test(t))) return false; // nessun segnale non-orologio
  if (IS_WATCH_OVERRIDE.test(t)) return false;                    // ma è chiaramente un orologio
  return true;                                                    // categoria non-orologio → scarta
}
