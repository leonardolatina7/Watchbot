...
async function runGoldScan(mode = 'all') {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  console.log(`\n[SCAN v11.15 ${mode==='subito'?'SUBITO':'COMPLETA'}] Oro: €${gold.toFixed(2)}/g | Platino: €${platinum.toFixed(2)}/g`);
...
app.post('/api/telegram/test',(req,res) => { tg(`⌚ <b>Watch Price Bot v11.15</b> — Test OK! 🟢`,req.body.chatId); res.json({ok:true}); });

app.get('/api/status',async(req,res) => {
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  res.json({
    status:'online',version:'11.15',
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  console.log(`\n⌚ Watch Price Bot v11.15 — porta ${PORT}`);
  ...
  if(process.env.TELEGRAM_TOKEN&&process.env.TELEGRAM_CHAT_ID){
    ...
    await tg(
      ...
      `\n<b>NOVITÀ v11.15 \u2014 dal trovare al GUADAGNARE:</b>\n`+
      `\u23F3 Ogni affare ora dice quanto e in quanto tempo rivendi: \u20AC/mese di capitale e %/mese. Il piccolo che gira batte il grande fermo.\n`+
      `\u{1F501}\u{1F48E} Etichetta FLIP (giro veloce, liquidita) vs TESORO (margine alto, tempi lunghi): sai che capitale impegni.\n`+
      `\u{1F4C9} Sottoprezzo relativo: ti avviso quando un pezzo costa meno della mediana degli altri uguali visti di recente, non solo sotto una soglia fissa.\n`+
      `\u{1F30D} Caccia geografica attiva: il bot pesca i modelli DOVE SONO FREDDI (tedeschi su fonti IT, italiani/FR su fonti DE) per rivenderli dove scottano.\n`+
      `\u{1F4CA} Nuovo endpoint /api/affari/roi: gli affari trovati ordinati per efficienza di capitale.\n`+
