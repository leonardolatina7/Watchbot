...
const QUERIES_BLOCK_C = [
  ...
  'Movado cronografo vintage',
  'orologio oro 18k carica manuale vintage', // generico oro
  'orologio eredita nonno vintage',          // svendite eredita
...
async function runGoldScan() {
  ...
  for (const query of activeQueries) {
    try {
      ...
      // FONTI: priorità ai marketplace di PRIVATI (dove si svende senza
      // sapere cosa si ha). Subito + eBay + Google Shopping (che pesca
      // Subito/Kijiji/eBay) + Vinted. Tolti i siti di dealer (Chrono24,
      // Catawiki, Watchfinder): lì il prezzo giusto lo sanno già.
      const sources = [
      ...
      for (const item of all) {
        ...
        if (priceEur>1500) continue; // sopra 1500 i privati sanno il valore: niente affari
