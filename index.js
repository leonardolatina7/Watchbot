...
async function searchVinted(query) {
  try {
    ...
    const r = await axios.get(`https://www.vinted.it/catalog?search_text=${encodeURIComponent(query)}&catalog[]=2165&price_from=500`, {
...
async function searchWallapop(query) {
  try {
    await sleep(900+Math.random()*500);
    const r = await axios.get(`https://api.wallapop.com/api/v3/general/search?keywords=${encodeURIComponent(query)}&category_ids=14000&min_sale_price=500&order_by=price_low_to_high`, {
...
async function searchMarktplaats(query) {
  try {
    await sleep(1000+Math.random()*500);
    const r = await axios.get(`https://www.marktplaats.nl/lrp/api/search?query=${encodeURIComponent(query)}&categoryId=363&priceFrom=500&sortBy=PRICE_ASC&limit=12`, {
      headers:{'User-Agent':rUA(),'Accept':'application/json'}, timeout:12000
    });
...
// ══════════════════════════════════════════════════════════════
// QUERY PER MARCA — larghe, così il database (108 modelli) riconosce il modello
// specifico tra i risultati. Poche ricerche, copertura ampia.
// Calibrate per 1.000/mese SerpAPI: 10 query × 3 scan/giorno × 30 = ~900/mese.
const QUERIES_BLOCK_A = [
  'orologio oro 18k vintage',
  'Omega vintage chronograph',
  'Rolex vintage',
  'Patek Philippe vintage',
  'Heuer vintage chronograph',
...
const QUERIES_BLOCK_B = [
  'orologio platino vintage',
  'Omega Seamaster vintage',
  'Rolex Daytona vintage',
  'Jaeger LeCoultre vintage',
  'Breitling Navitimer vintage',
  'IWC vintage pilot',
  'Seiko vintage diver chronograph',
  'Grand Seiko vintage',
  'Girard Perregaux Laureato vintage',
  'Enicar Sherpa Doxa vintage',
];
let scanCounter = 0;
function getGoldQueries() {
  scanCounter++;
  return scanCounter % 2 === 1 ? QUERIES_BLOCK_A : QUERIES_BLOCK_B;
}
...
async function runGoldScan() {
  ...
  console.log(`[SCAN v11.1] Blocco query: ${scanCounter % 2 === 1 ? 'A' : 'B'} (${activeQueries.length} query)`);
  for (const query of activeQueries) {
    try {
      ...
      const sources = [
        searchEbayAllCountries, searchChrono24RSS, searchCatawiki,
        searchSubito, searchLeboncoin, searchVinted,
        searchWallapop, searchRicardo, searchSerpAPILocal,
      ];
      let all = [];
      for (const src of sources) {
        try {
          ...
          if (Array.isArray(r) && r.length) {
            ...
            for (const it of r) {
              all.push({ platform: it.platform, title: it.title, price: it.price, currency: it.currency, url: it.url, location: it.location || '' });
            }
          }
      ...
      for (const item of all) {
        ...
        // ── VALUTAZIONE VINTAGE (qualsiasi materiale, anche acciaio) ──
        ...
        if (vintage && vintage.isDeal && !seenUrls.has(item.url)) {
          ...
          const grailTag = vintage.grail ? '👑 GRAIL ' : '';
          const stars = '⭐'.repeat(Math.min(Math.round(vintage.desirability/2),5));
          await tg(
            `🏺 <b>${grailTag}OCCASIONE VINTAGE!</b>\n\n`+
            `⌚ ${item.title?.slice(0,65)}\n`+
            `🔧 <b>${vintage.brand} ${vintage.model}</b>\n`+
            `⚙️ ${vintage.caliber} · ${vintage.years}\n`+
            `🪙 ${vintage.material}\n`+
            `💰 Prezzo: <b>€${priceEur.toLocaleString('it-IT')}</b>\n`+
            `📊 Valore mercato: <b>€${vintage.valueLow.toLocaleString('it-IT')}–${vintage.valueHigh.toLocaleString('it-IT')}</b>\n`+
            (vintage.discountVsLow>0?`📉 <b>−${vintage.discountVsLow}% sotto il minimo di mercato!</b>\n`:'')+
            `${stars} Desiderabilità ${vintage.desirability}/10\n`+
            `🏪 ${item.platform}${item.location?` · 📍 ${item.location}`:''}\n\n`+
            `💡 ${vintage.note}\n\n`+
            `<a href="${item.url}">👉 VEDI ANNUNCIO</a>`
          );
          console.log(`[VINTAGE 🏺] ${vintage.brand} ${vintage.model} €${priceEur} (val €${vintage.valueLow}+, −${vintage.discountVsLow}%)`);
          continue; // già processato come vintage
        }

        // ── ANALISI CLAUDE (fallback intelligente) ──
        // Se il database locale NON ha riconosciuto un'occasione, chiediamo a Claude.
        // Solo per pezzi ≥800€ (evita rumore economico) e se Claude è configurato.
        // Claude ragiona sul titolo come farebbe un esperto: marca, calibro, valore.
        if (claudeAnalyst.isConfigured() && priceEur >= 800 && !vintage) {
          const ai = await claudeAnalyst.analyzeListing(item.title, priceEur);
          if (ai && ai.isInteresting) {
            ...
            db.vintageDeals.push({
              ...
              grail:ai.isGrail||false, url:item.url, location:item.location||'',
              source:'claude', foundAt:new Date().toISOString(),
            });
            const grailTag = ai.isGrail ? '👑 GRAIL ' : '';
            const stars = '⭐'.repeat(Math.min(Math.round((ai.desirability||0)/2),5));
            const confLabel = ai.confidence==='high'?'✅ identificazione sicura':ai.confidence==='medium'?'🟡 identificazione probabile':'⚠️ identificazione incerta';
            const strengthEmoji = ai.dealStrength==='FORTE'?'🔥🔥🔥':ai.dealStrength==='BUONO'?'🔥🔥':'🔥';
            await tg(
              `🤖 <b>${grailTag}AFFARE ${ai.dealStrength||''} ${strengthEmoji}</b>\n\n`+
              `⌚ ${item.title?.slice(0,65)}\n`+
              (ai.brand?`🔧 <b>${ai.brand}${ai.model&&ai.model!=='null'?' '+ai.model:''}</b>\n`:'')+
              (ai.caliber&&ai.caliber!=='null'?`⚙️ ${ai.caliber}\n`:'')+
              (ai.material&&ai.material!=='sconosciuto'?`🪙 ${ai.material}\n`:'')+
              `💰 Prezzo: <b>€${priceEur.toLocaleString('it-IT')}</b>\n`+
              (ai.valueLow?`📊 Valore stimato: <b>€${ai.valueLow.toLocaleString('it-IT')}–${(ai.valueHigh||ai.valueLow).toLocaleString('it-IT')}</b>\n`:'')+
              (ai.discountVsLow>0?`📉 <b>−${ai.discountVsLow}% sotto la stima</b>\n`:'')+
              (ai.marginEur>0?`💵 Margine potenziale: <b>~€${ai.marginEur.toLocaleString('it-IT')}</b>\n`:'')+
              (stars?`${stars} Desiderabilità ${ai.desirability}/10\n`:'')+
              `${confLabel}\n`+
              (ai.redFlags&&ai.redFlags!=='null'?`🚩 <b>Attenzione:</b> ${ai.redFlags}\n`:'')+
              `🏪 ${item.platform}${item.location?` · 📍 ${item.location}`:''}\n\n`+
              `💡 ${ai.reasoning||''}\n\n`+
              `⚠️ Stima da titolo, non da foto. Guarda le foto e verifica condizioni, originalità del quadrante e calibro prima di comprare.\n\n`+
              `<a href="${item.url}">👉 VEDI ANNUNCIO</a>`
            );
            console.log(`[CLAUDE 🤖] ${ai.dealStrength} ${ai.brand} ${ai.model} €${priceEur} (stima €${ai.valueLow}, margine €${ai.marginEur}, ${ai.confidence})`);
            continue;
          }
        }

        // ── VALUTAZIONE METALLO (oro/platino) ──
        ...
        if (metal.isArbitrage) {
          ...
          await tg(
            ...
            `   ${metal.pureMetalGrams}g × €${metal.spotPrice}/g\n`+
            `📉 <b>−${metal.diffPct}% sotto valore metallo!</b>\n`+
            `💵 Guadagno: <b>€${metal.diff.toLocaleString('it-IT')}</b>\n`+
            `🏪 ${item.platform}${item.location?` · 📍 ${item.location}`:''}\n`+
            (metal.confidence==='high'?`✅ Peso certificato (scheda tecnica)\n`:
             metal.confidence==='medium'?`🟡 Peso stimato con alta probabilità\n`:
             `⚠️ Peso stimato — verifica prima!\n`)+
            `\n🔍 <a href="${metal.verifyLink||'https://watchcharts.com'}">Verifica peso su WatchCharts</a>\n`+
            `\n<a href="${item.url}">👉 VEDI ANNUNCIO</a>`
          );
...
app.get('/api/claude/check',async(req,res)=>{
  ...
  const result=await claudeAnalyst.analyzeListing(title, price?parseFloat(price):5000);
...
// ── CRON ─────────────────────────────────────────────────────
// Scansioni distribuite su ore DIVERSE per non sovrapporsi mai in memoria.
// Oro: 00,08,16 · Hype: 04,12,20 · Discovery: 02,14 — mai contemporanee.
cron.schedule('0 0,8,16 * * *',()=>runGoldScan().catch(()=>{}));
cron.schedule('0 4,12,20 * * *',()=>runHypeScan().catch(()=>{}));
cron.schedule('0 2,14 * * *',()=>runDiscoveryScan().catch(()=>{}));
