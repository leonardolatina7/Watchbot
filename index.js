...
async function getGoldPrice() {
  ...
  // ripiego realistico (agg. 2026): oro fino ~95 €/g. Meglio non sovrastimare.
...
function detectMetal(t) {
  ...
  // ORO SOLO PARZIALE / non massiccio -> non è arbitraggio metallo
  ...
  // ORO: le parole vanno cercate come PAROLE INTERE, non dentro altri numeri.
  // (Bug: "750" faceva match dentro "Valjoux 7750", "585" dentro calibri, ecc.)
  // "750"/"585"/"375" contano solo se isolati o legati a oro/gold/k/carati.
  ...
  // numeri di caratura SOLO come parola intera (confine non-cifra ai due lati)
...
async function calcMetal(title, priceEur) {
  ...
  // PAVIMENTO PRUDENTE: valore dell'oro col peso MINIMO dell'intervallo.
  // Se il prezzo è sotto questo, sei coperto anche nello scenario peggiore
  // (cassa leggera) -> puoi decidere in fretta senza aspettare il peso.
  ...
  const safeFloor = valueLow;                 // sotto questo = sicuro anche al minimo
...
async function searchEbayAllCountries(query) {
  const domains = [
    ...
    {dom:'com', curr:'USD', lang:'en-US'},   // USA/globale
    // Venditori ASIATICI (Giappone, Hong Kong, Singapore...) che spediscono:
    // filtro "worldwide" su eBay.com. Alla pari con gli altri, nessuna priorità.
      ...
      for (const item of all) {
        ...
        if (detectMetal(item.title)) {
          ...
          if (metal && (metal.isArbitrage || metal.isNear)) {
            ...
            // metallo ma non "sicuro" (peso non certo): se il prezzo è sotto
            // il PAVIMENTO PRUDENTE (oro col peso minimo stimato), vale la pena
            // correre -> alert rapido. Altrimenti salvato per il sito + Claude.
            if (!metal.weightKnown && metal.underSafeFloor && metal.confidence !== 'high') {
              ...
              await tg(
                ...
                `\u231A ${item.title?.slice(0,65)}\n`+
                `\u{1F4B0} Chiede: <b>\u20AC${priceEur.toLocaleString('it-IT')}</b>\n`+
                `\u{1F4CF} Oro stimato: <b>${g}</b> (da dimensioni, NON certo)\n`+
                `\u{1F4B0} Vale come oro circa: <b>\u20AC${metal.valueLow.toLocaleString('it-IT')}\u2013\u20AC${metal.valueHigh.toLocaleString('it-IT')}</b>\n`+
                `\u2705 Anche col peso MINIMO l'oro vale \u20AC${metal.safeFloor.toLocaleString('it-IT')}: tu paghi meno \u2192 coperto pure nello scenario peggiore.\n\n`+
                `\u{1F3AF} <b>Mossa veloce:</b> se devi correre, sei comunque protetto dall'oro. Per chiudere meglio chiedi il <b>peso della cassa</b> e usa il calcolatore.\n`+
                `\u{1F9E0} Stima da dimensioni: il peso esatto cambia da cassa a cassa.\n`+
                `\u{1F3EA} ${item.platform}${item.location?` \u00B7 \u{1F4CD} ${item.location}`:''}\n\n`+
                `<a href="${item.url}">\u{1F449} VEDI ANNUNCIO</a>\n\n`+
                `\u2796\u2796\u2796\n`+
                feedbackLinks(null, item.url, item.title, priceEur)
              );
              console.log(`[ORO-RAPIDO] ${item.title?.slice(0,40)} \u20AC${priceEur} <= floor \u20AC${metal.safeFloor}`);
              continue;
            }
        ...
        if (claudeAnalyst.isConfigured()) {
          ...
          if (ai && ai.isInteresting) {
            ...
            if ((ai.marginEur||0) >= 150 && (ai.discountVsLow||0) >= 20) {
              ...
              // PARTICOLARE: pezzo raro da valutare a mano
              verdetto = `\u{1F50E} <b>PEZZO PARTICOLARE</b> \u2014 raro/ricercato, valuta bene: puo\u0300 valere molto piu\u0300 del normale`;
            } else {
              // Affare moderato: comprare bene, rivendere con calma
...
app.get('/api/gold-price', async(req,res) => {
  ...
  res.json({
    oro:p?Math.round(p*100)/100:null,
    platino:pt?Math.round(pt*100)/100:null,
    palladio:pd?Math.round(pd*100)/100:null,
    pricePerGram:p?Math.round(p*100)/100:null,
    history:db.goldPrices.slice(-48).reverse()
  });
});

// CALCOLATORE ORO: pagina da tenere nei preferiti del telefono.
// Inserisci peso e caratura -> valore di fusione + offerta massima consigliata.
// Usa il prezzo oro LIVE gia presente nel bot.
app.get('/api/oro', async(req,res) => {
  const live = await getGoldPrice().catch(()=>null); // €/g oro FINO (999)
...
<title>Calcolatore Oro - Watch Price Bot</title>
<style>
  body{font-family:-apple-system,Segoe UI,Roboto,sans-serif;background:#11161f;color:#f3f1ea;margin:0;padding:22px;line-height:1.5}
  .card{max-width:560px;margin:0 auto}
  h1{font-size:21px;margin:0 0 4px}
  .live{color:#e8b923;font-weight:700;margin:0 0 18px;font-size:15px}
  label{display:block;margin:14px 0 5px;font-weight:600;font-size:15px}
  input,select{width:100%;padding:13px;background:#1b2230;color:#f3f1ea;border:1px solid #3a4456;border-radius:9px;font-size:16px;box-sizing:border-box}
  .row{display:flex;gap:10px}.row>div{flex:1}
  button{width:100%;padding:15px;margin-top:18px;border:0;border-radius:10px;font-size:17px;font-weight:700;background:#e8b923;color:#11161f;cursor:pointer}
  .out{background:#1b2230;border:1px solid #3a4456;border-radius:11px;padding:16px;margin-top:18px;display:none}
  .big{font-size:26px;font-weight:800;color:#4ccf6a;margin:6px 0}
  .lab{font-size:13px;color:#9aa3b2;text-transform:uppercase;letter-spacing:.5px}
  .max{font-size:22px;font-weight:800;color:#e8b923;margin:6px 0}
  .note{font-size:13px;color:#9aa3b2;margin-top:14px}
  .hint{font-size:13px;color:#9aa3b2;margin-top:3px}
</style></head><body><div class="card">
  <h1>🥇 Calcolatore Oro</h1>
  <p class="live">Prezzi LIVE oggi:<br>🥇 Oro fino (24k/999) ${goldFine} €/g<br>&nbsp;&nbsp;&nbsp;→ <b>18k (750) = ${Math.round(goldFine*0.750*100)/100} €/g</b> · 14k (585) = ${Math.round(goldFine*0.585*100)/100} €/g · 9k (375) = ${Math.round(goldFine*0.375*100)/100} €/g<br>⚪ Platino ${ptFine} €/g · ⚫ Palladio ${pdFine} €/g</p>

  <label>Peso (grammi)</label>
  <input id="peso" type="number" inputmode="decimal" placeholder="es. 46.18" />
  <div class="hint">Se hai solo il peso LORDO (con movimento e vetro), spunta sotto.</div>

  <div class="row">
    <div>
      <label>Caratura</label>
      <select id="kt">
        <option value="0.750">18 kt (750) - oro svizzero/italiano tipico</option>
        <option value="0.585">14 kt (585)</option>
        <option value="0.375">9 kt (375)</option>
        <option value="0.916">22 kt (916)</option>
        <option value="0.999">24 kt (999) - oro puro</option>
        <option value="0">PLACCATO / dorato (vale 0 come oro)</option>
      </select>
    </div>
  </div>

  <label style="margin-top:14px"><input type="checkbox" id="lordo" style="width:auto;margin-right:8px">È peso LORDO (togli ~30% per movimento/vetro)</label>

  <button onclick="calc()">Calcola</button>

  <div class="out" id="out">
    <div class="lab">Valore di fusione (solo oro)</div>
    <div class="big" id="melt">—</div>
    <div class="lab">Offerta MAX consigliata (oro × 0,80)</div>
    <div class="max" id="maxoff">—</div>
    <div class="note" id="note"></div>
  </div>

  <p class="note">Regole: in asta ricordati di aggiungere la commissione (21-24%) al prezzo battuto. Compra solo se il costo totale resta sotto il valore di fusione. Placcato = niente paracadute oro.</p>

  <div style="background:#1b2230;border:1px solid #3a4456;border-radius:11px;padding:14px 16px;margin-top:16px">
    <div class="lab" style="margin-bottom:8px">📏 Quanto oro c'è di solito (oro NETTO 18k) — ordine di grandezza</div>
    <div style="font-size:14px;line-height:1.9">
      Donna / piccolo (≤32mm): <b>8–15 g</b><br>
      Dress uomo 33–34mm: <b>12–20 g</b><br>
      Dress / crono 36–38mm: <b>16–28 g</b><br>
      Sportivo / grande 39–41mm: <b>24–40 g</b><br>
      Oversize 42–44mm: <b>35–60 g</b><br>
      Da tasca: <b>25–45 g</b>
    </div>
    <div class="note" style="margin-top:10px">Sono intervalli REALISTICI per farti un'idea al volo. Il numero esatto cambia da cassa a cassa: chiedi sempre il peso e mettilo qui sopra per il valore preciso.</div>
  </div>
</div>
<script>
  var FINE=${goldFine};
  function calc(){
    ...
    var lordo=document.getElementById('lordo').checked;
    if(!peso||peso<=0){alert('Inserisci il peso');return;}
    var pesoOro=lordo?peso*0.70:peso;
    var melt=pesoOro*kt*FINE;
    var out=document.getElementById('out');out.style.display='block';
    if(kt===0){
      document.getElementById('melt').textContent='0 € (placcato)';
      document.getElementById('maxoff').textContent='Vale solo come orologio';
      document.getElementById('note').textContent='Placcato/dorato: niente valore oro. Conta solo se è un modello ricercato.';
      return;
    }
    var maxoff=melt*0.80;
    document.getElementById('melt').textContent=Math.round(melt).toLocaleString('it-IT')+' €';
    document.getElementById('maxoff').textContent='~'+Math.round(maxoff).toLocaleString('it-IT')+' €';
    var d='Oro netto calcolato: '+(Math.round(pesoOro*10)/10)+' g a '+(kt*1000)+'/1000.';
    if(lordo) d+=' (peso lordo -30%: stima GENEROSA, sui cronografi il movimento pesa di più. Chiedi sempre il peso della SOLA cassa prima di offrire sul serio)';
...
app.get('/api/fb', (req, res) => {
  ...
  if (a === 'bl' && b) {
    ...
    return res.send(fbPage('🚫 Bloccato',
      `Non riceverai più <b>${brand}</b>.<br><br>`+
      `<b style="color:#e8b923">Per renderlo PERMANENTE</b> (anche dopo i riavvii del bot), copia questa riga e incollala su Render nella variabile <b>BLACKLIST_BRANDS</b>:<br><br>`+
      `<textarea readonly style="width:100%;height:70px;background:#1b2230;color:#f3f1ea;border:1px solid #3a4456;border-radius:8px;padding:8px;font-size:13px">${lista}</textarea>`));
  }
  if (a === 'dz' && u) {
    const url = String(u);
    ...
    return res.send(fbPage('👎 Scartato',
      `Questo singolo annuncio non tornerà più.<br><br>`+
      `<b style="color:#e8b923">Vuoi eliminare TUTTI gli annunci di questo marchio?</b> Usa invece il pulsante <b>🚫 Niente [marchio]</b> nell'alert: \"Scarta\" toglie solo questo annuncio, \"Niente marchio\" li blocca tutti.`));
  }
  if (a === 'lk') {
...
// Pagina "Analizza con Claude": prepara un testo pronto da copiare e
// incollare nella chat con Claude, così analizzate insieme il pezzo.
app.get('/api/analyze', (req, res) => {
  ...
  const testo =
    `Ciao Claude, analizziamo insieme questo orologio come al solito.\n\n`+
    `Marca: ${b || '(da capire)'}\n`+
    `Annuncio: ${t}\n`+
    `Prezzo richiesto: ${prezzo}\n`+
    `Link: ${u}\n\n`+
    `Dimmi: è roba mia (crono/diver/oro vintage da uomo)? È vero vintage o moderno? `+
    `Il prezzo è un affare? Quadrante originale o rischio redial? Cassa nazionale o oro vero? `+
    `Cosa chiedere al venditore e a quanto trattare? Verdetto: compro, tratto a X, o lascio?`;
  const esc = (s) => s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
  const testoJs = JSON.stringify(testo);
  res.send(`<!doctype html><html lang="it"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Analizza con Claude</title>
<style>
  body{font-family:-apple-system,Segoe UI,Roboto,sans-serif;background:#11161f;color:#f3f1ea;margin:0;padding:24px;line-height:1.5}
  .card{max-width:640px;margin:0 auto}
  h1{font-size:22px;margin:0 0 6px}
  p.sub{color:#9a7b2e;margin:0 0 18px;font-weight:600}
  textarea{width:100%;height:230px;background:#1b2230;color:#f3f1ea;border:1px solid #3a4456;border-radius:10px;padding:12px;font-size:15px;box-sizing:border-box}
  button{width:100%;padding:15px;margin-top:12px;border:0;border-radius:10px;font-size:17px;font-weight:700;cursor:pointer}
  .copy{background:#9a7b2e;color:#fff}
  .open{background:#2f6fed;color:#fff;text-decoration:none;display:block;text-align:center;box-sizing:border-box}
  .ok{color:#4ccf6a;text-align:center;margin-top:10px;font-weight:700;min-height:20px}
  .steps{background:#1b2230;border:1px solid #3a4456;border-radius:10px;padding:12px 16px;margin-top:16px;font-size:14px;color:#cfc8b8}
</style></head><body><div class="card">
  <h1>🔍 Analizza con Claude</h1>
  <p class="sub">Copia il testo e incollalo nella chat con Claude</p>
  <textarea id="t" readonly>${esc(testo)}</textarea>
  <button class="copy" onclick="cp()">📋 Copia il testo</button>
  <a class="open" href="https://claude.ai/new">💬 Apri Claude</a>
  <div class="ok" id="ok"></div>
  <div class="steps">
    <b>Come fare:</b><br>
    1. Tocca <b>Copia il testo</b><br>
    2. Tocca <b>Apri Claude</b> (o vai nell'app Claude)<br>
    3. <b>Incolla</b> il testo e <b>allega lo screenshot</b> dell'annuncio<br>
    4. Analizzate insieme come al solito 🎯
  </div>
</div>
<script>
  function cp(){
    var v=${testoJs};
    navigator.clipboard.writeText(v).then(function(){document.getElementById('ok').textContent='✅ Copiato! Ora apri Claude e incolla.';})
    .catch(function(){var ta=document.getElementById('t');ta.removeAttribute('readonly');ta.select();document.execCommand('copy');document.getElementById('ok').textContent='✅ Copiato!';});
  }
</script></body></html>`);
});
