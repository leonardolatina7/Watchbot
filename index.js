...
function getGoldQueries() {
  ...
  // ora che si scansiona ogni 4h (non ogni 30min), ogni ciclo cerca PIÙ AMPIO:
  // due blocchi tematici diversi a rotazione...
  const themed = [...rotating[scanCounter % rotating.length], ...rotating[(scanCounter+3) % rotating.length]];
  // ...e 15 query del pool enciclopedico a finestra scorrevole (era 5):
  // in ~4 cicli copriamo TUTTO il pool, cioè tutto quello che sappiamo.
  const N = 15;
...
async function runGoldScan(mode = 'all') {
  const gold = await getGoldPrice();
  const platinum = await getPlatinumPrice();
  console.log(`\n[SCAN v11.13 ${mode==='subito'?'SUBITO':'COMPLETA'}] Oro: €${gold.toFixed(2)}/g | Platino: €${platinum.toFixed(2)}/g`);
  ...
  for (const query of activeQueries) {
    try {
      const sources = mode === 'subito'
        ...
        : [
            searchMarktplaats,        // funziona bene, non usa SerpAPI
            searchChrono24RSS,        // RSS stabile, no scraping
            searchSubito,             // JSON Next.js, abbastanza stabile
            searchEbayAllCountries,   // HTML moderno, può essere bloccato
            searchCatawiki,           // aste, funziona spesso
            searchVinted,             // moda/usato, occasional
            // SerpAPI: solo se quota disponibile (100/mese piano free)
            ...(serpQuotaOk() ? [(qq)=>searchSerpAPI(qq,'it'), searchSerpAPILocal] : []),
          ];
      let all = [];
      ...
      for (const item of all) {
        ...
        if (detectMetal(item.title)) {
          ...
          if (metal && (metal.isArbitrage || metal.isNear)) {
            ...
            // ── FALLBACK: arbitraggio/near contato ma non rientrato nei rami
            //    sopra (es. confidenza media, o peso stimato sotto soglia).
            //    Prima cadeva nel vuoto: contato ma MAI inviato. Ora avvisa
            //    sempre, segnalando che è da verificare.
            {
              seenUrls.add(nu); markAlerted(fp); saveState();
              const emoji = metal.metal==='platinum'?'\u{1F518}':'\u{1F947}';
              const name = metal.metal==='platinum'?'platino':'oro';
              const g = metal.gramsLow && metal.gramsHigh ? `${metal.gramsLow}\u2013${metal.gramsHigh}g` : `~${metal.pureMetalGrams}g`;
              await tg(
                `${emoji} <b>METALLO \u2014 DA VERIFICARE</b>\n\n\u231A ${item.title?.slice(0,65)}\n`+
                `\u{1F4B0} Chiede: <b>\u20AC${priceEur.toLocaleString('it-IT')}</b>\n`+
                `\u{1F48E} ${name==='platino'?'Platino':'Oro'} stimato: <b>${g}</b> \u2192 vale ~\u20AC${metal.metalValue.toLocaleString('it-IT')}\n`+
                (metal.diffPct>0?`\u{1F4C9} \u2212${metal.diffPct}% sotto il metallo `:`\u{1F4CA} ${Math.abs(metal.diffPct)}% sopra il metallo `)+
                `(confidenza ${metal.confidence})\n`+
                `\u26A0\uFE0F Peso e caratura da confermare col venditore (foto fondello + punzoni + peso).\n`+
                `\u{1F3EA} ${item.platform}${item.location?` \u00B7 \u{1F4CD} ${item.location}`:''}\n\n`+
                `<a href="${item.url}">\u{1F449} VEDI ANNUNCIO</a>\n\n\u2796\u2796\u2796\n`+
                feedbackLinks(null, item.url, item.title, priceEur)
              );
...
app.post('/api/telegram/test',(req,res) => { tg(`⌚ <b>Watch Price Bot v11.13</b> — Test OK! 🟢`,req.body.chatId); res.json({ok:true}); });

app.get('/api/status',async(req,res) => {
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  res.json({
    status:'online',version:'11.13',
...
// ── CRON ──
// UNA scansione COMPLETA ogni 4 ore (00,04,08,12,16,20): cerca su TUTTE le
// fonti e TUTTE le query enciclopediche che conosciamo. Meglio cercare bene
// ogni 4 ore che male ogni 30 minuti.
...
app.listen(PORT,'0.0.0.0',async()=>{
  ...
  const gold=await getGoldPrice().catch(()=>null);
  const platinum=await getPlatinumPrice().catch(()=>null);
  console.log(`\n⌚ Watch Price Bot v11.13 — porta ${PORT}`);
  ...
  if(process.env.TELEGRAM_TOKEN&&process.env.TELEGRAM_CHAT_ID){
    ...
    await tg(
      ...
      `\n<b>NOVITÀ v11.13:</b> scansione ora ogni 4 ore (non più ogni 30 min) ma CERCA PIÙ AMPIO — in 24h copre il 100% di tutto quello che sappiamo (62 query enciclopediche + parenti poveri delle leggende + fascia media calda). Meno token, copertura totale. Alert metallo sempre inviati, solo occasioni vere.\n`+
