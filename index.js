      ...
      for (const item of all) {
        ...
        if (claudeAnalyst.isConfigured() && priceEur >= 800 && !vintage) {
          ...
          if (ai && ai.isInteresting) {
            ...
            const photoLabel = ai.sawImage ? '📸 Claude ha visto la foto' : '📝 Solo descrizione (foto non disponibile)';
            ...
            await tg(
              ...
              (ai.condition&&ai.condition!=='null'?`🔍 Condizioni: ${ai.condition}\n`:'')+
              `💰 Prezzo: <b>€${priceEur.toLocaleString('it-IT')}</b>\n`+
              ...
              `🏪 ${item.platform}${item.location?` · 📍 ${item.location}`:''}\n\n`+
              `💡 ${ai.reasoning||''}\n\n`+
              (ai.sawImage?`👉 Claude ha analizzato la foto, ma controlla TUTTE le immagini dell'annuncio prima di comprare.\n\n`:`⚠️ Stima dal solo titolo. Guarda bene le foto prima di comprare.\n\n`)+
              `<a href="${item.url}">👉 VEDI ANNUNCIO</a>`
            );
            console.log(`[CLAUDE 🤖] ${ai.dealStrength} ${ai.brand} ${ai.model} €${priceEur} (foto:${ai.sawImage?'sì':'no'}, margine €${ai.marginEur}, ${ai.confidence})`);
            continue;
          }
        }
