...
/**
 * PRE-FILTRO a costo zero: ritorna true se l'annuncio merita un'analisi Claude,
 * false se è spazzatura ovvia (occhiali, accessori, top moderni senza margine...).
 * Estratto come funzione propria così lo usano sia analyzeListing sia la
 * modalità dry-run (test a secco), per testare il filtro senza spendere token.
 */
function passesPreFilter(title) {
  ...
  const SCARTA = [
    ...
    'smartwatch','apple watch','samsung','fitbit','garmin','huawei','xiaomi','amazfit','smart watch',
    // NON sono orologi: occhiali, gioielli, accessori (causa #1 dei falsi)
    'occhiali','occhiale','sunglasses','eyewear','sonnenbrille','lunettes','gafas',
    'anello','collana','bracciale oro','catenina','ciondolo','orecchini','spilla','gemelli',
    'penna','stilografica','accendino','lighter','portafoglio','wallet','borsa','foulard','cravatta','sciarpa','profumo',
    // ricambi / parti / contorni
    'cinturino','bracciale ricambio','solo cinturino','solo quadrante','solo cassa','solo movimento',
    'solo fondello','solo corona','solo lancette','solo vetro','parti di ricambio','for parts',
    'scatola vuota','box vuota','solo box','solo scatola','teca','espositore','cofanetto vuoto',
    'sveglietta','timer','cronometro da tasca da muro','orologio da muro',
    // falsi / robaccia
    'replica','first copy','tipo rolex','stile rolex','no brand','senza marca','homage','omaggio a',
    'giocattolo','bambino','cartoon','disney','plastica','silicone','digitale anni','lcd',
  ];
  if (SCARTA.some(k => t.includes(k))) return false;

  // serve almeno un indizio di orologio da polso: parole-orologio o marca nota
  const MARCA_OROLOGIO = /(rolex|omega|cartier|tudor|longines|zenith|heuer|breitling|iwc|jaeger|patek|audemars|vacheron|panerai|seiko|citizen|tissot|eberhard|universal|enicar|nivada|gallet|eterna|girard|movado|doxa|vulcain|favre|wittnauer|wakmann|excelsior|lemania|minerva|angelus|mido|certina|cortebert|tavannes|cyma|buren|glashutte|junghans|stowa|hanhart|tutima|glycine|hamilton|bulova|gubelin|juvenia|helvetia|revue thommen|martel|kelek|chezard|ernest borel|roamer|marvin|smiths|vertex|grana|timor|dodane|auricoste|airain|yema|sinn|fortis|lip|paul picot|mathey|record|moeris|election)/i;
  const indizioOrologio = /(orolog|montre|uhr|reloj|wrist\s?watch|chronograph|cronograf|automatic|automatico|carica manuale|manual wind|diver|subacqueo|\bwatch\b)/i.test(t) || MARCA_OROLOGIO.test(t);
  if (!indizioOrologio) return false;

  const segnaleVintage = /(anni\s?[3-7]0|19[3-7]0|vintage|cal\.\s?\d|valjoux|venus|lemania|landeron|pre.?moon|gilt|tropical)/i.test(t);

  // marchi top moderni senza segnale vintage = niente margine, niente Claude
  const HItier = /(rolex|patek|audemars|vacheron|cartier|panerai|hublot|richard mille|a\.?\s?lange|jaeger.?lecoultre|breguet)/i.test(t);
  if (HItier && !segnaleVintage) return false;

  // acciaio-oro / bicolore moderno = né pavimento oro né interesse vintage
  const acciaioOro = /(acciaio.?oro|oro.?acciaio|two.?tone|bicolor|bicolore|steel.?and.?gold|gold.?and.?steel|stahl.?gold|acier.?or)/i.test(t);
  if (acciaioOro && !segnaleVintage) return false;
...
async function analyzeListing(title, priceEur, imageUrl) {
  ...
  if (!passesPreFilter(title)) return null;
...
function evaluateWithPrice(analysis, priceEur) {
  ...
  out.isInteresting = affareVero || out.studyPick;
  out.isRealDeal = affareVero; // distingue affare vero da semplice studio
...
module.exports = { analyzeListing, isConfigured, getUsage, passesPreFilter };
