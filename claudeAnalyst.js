...
// ── THROTTLE ANTI-429: il piano gratis Groq limita le richieste AL MINUTO
//    (rate limit RPM, non solo al giorno). Durante uno scan con decine di
//    query il bot sparava le chiamate tutte insieme → 429 immediato anche
//    a quota giornaliera piena. Qui garantiamo una distanza minima tra una
//    chiamata Groq e la successiva, così restiamo sotto il tetto/minuto.
//    Regolabile con GROQ_MIN_INTERVAL_MS (default 2200ms ≈ max ~27 req/min). ──
const GROQ_MIN_INTERVAL_MS = parseInt(process.env.GROQ_MIN_INTERVAL_MS || '2200');
let _lastGroqCall = 0;
const _sleep = ms => new Promise(r => setTimeout(r, ms));
async function throttleGroq() {
  const wait = _lastGroqCall + GROQ_MIN_INTERVAL_MS - Date.now();
...
- 16) RAGIONA DA INVESTITORE / PORTAFOGLIO (regola che governa scenari ed evRating): valuti come un'azione, non come una singola scommessa. Tre scenari a 5 anni — pessimistico / BASE / ottimistico — e la BASE è quella che pesa (~65% dei casi SE comprato bene). Principi: (a) la BASE va tenuta CONSERVATIVA, sotto l'entusiasmo: è la conseguenza meccanica del comprare bene, non una speranza. (b) NON prezzare le code: né il +300% miracoloso né il disastro — sulla lunga si annullano, decidi sul centro. (c) Il filtro decisivo: un pezzo è un buon affare SOLO se rende il giusto già nello scenario BASE conservativo; se "funziona" solo nell'ottimistico, vuol dire che NON è comprato bene a quel prezzo → evRating basso e nel reasoning di' che serve uno sconto. (d) Questo NON penalizza l'acciaio: un acciaio col MOTORE acceso (catalizzatore/liquidità) comprato a prezzo onesto regge benissimo la base → può essere high. Quello che resta fuori è il pezzo SENZA motore pagato pieno (base piatta = magazzino, non investimento), oro o acciaio che sia. (e) L'upside oltre la base, se il lavoro è fatto bene (studio, comparabili, dial verificato), è un BONUS gratis non prezzato: meglio sotto-promettere.`;
  ...
  // Funzione interna che fa UNA chiamata a Groq (usata anche per il retry)
  async function callGroq() {
    await throttleGroq(); // rispetta la distanza minima tra chiamate (anti-429)
  ...
  try {
    ...
    try {
      ...
      // 429 = rate limit AL MINUTO: aspetta e riprova UNA volta. Groq manda
      // spesso 'retry-after' in secondi; altrimenti aspettiamo un tempo fisso.
      if (e1.response?.status === 429) {
        const ra = parseFloat(e1.response.headers?.['retry-after']);
        const waitMs = (ra && !isNaN(ra)) ? Math.min(ra * 1000, 15000) : 8000;
        console.error(`[GROQ] 429 rate-limit: attendo ${Math.round(waitMs/1000)}s e riprovo…`);
        await _sleep(waitMs);
        _lastGroqCall = 0; // forza subito la prossima chiamata dopo l'attesa
        r = await callGroq();
      } else {
        throw e1;
      }
    ...
    else console.error('[GROQ]', status ? `${status}: ${JSON.stringify(e.response?.data||'').slice(0,200)}` : e.message);
    return null;
  }
}
