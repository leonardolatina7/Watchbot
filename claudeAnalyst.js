...
function evaluateWithPrice(analysis, priceEur) {
  ...
  out.isInteresting = idAffidabile && senzaAllarmi && (margineForte || grailScontato || sleeperDiQualita || venditoreIgnaro || tesoroOscuro);

  // Etichetta forza dell'affare (con boost per i segnali genio)
  if (out.isInteresting) {
    let base = (discountVsLow >= 50 || marginEur >= 1000) ? 'FORTE'
