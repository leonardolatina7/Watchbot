...
// ============================================================================
async function claudeCall(contentBlocks, { maxTokens = 1200, temperature = 0.2, system = null } = {}) {
  const body = {
    model: CLAUDE_MODEL,
...
async function textComplete(prompt, opts = {}) {
  ...
  if (CLAUDE_KEY) {
    try {
      await _throttle();
      const out = await claudeCall([{ type: 'text', text: prompt }], { maxTokens: o.maxTokens, temperature: o.temperature });
      stats.claudeOk++;
      return out;
    } catch (e) {
      stats.claudeFail++;
      console.error('[visionEngine] Claude testo fallito:', e.response?.status || '', e.message, '→ fallback Gemini/Groq');
    }
  }
...
async function visionComplete(imageUrl, prompt, opts = {}) {
  ...
  if (CLAUDE_KEY) {
    try {
      ...
      const out = await claudeCall(blocks, { maxTokens: o.maxTokens, temperature: o.temperature });
      stats.claudeOk++;
      return out;
    } catch (e) {
      stats.claudeFail++;
      console.error('[visionEngine] Claude vision fallito:', e.response?.status || '', e.message, '→ fallback Gemini/Groq');
    }
  }
